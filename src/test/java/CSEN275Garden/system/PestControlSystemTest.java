...
public class PestControlSystemTest {
    ...
    @BeforeEach
    void setUp() {
        garden = new GardenPlot(9, 9);
        pestControl = new PestControlSystem(garden);
        
        // Add some plants for testing
        garden.addPlant(new Flower(new GridPosition(0, 0), "Lily"));
        garden.addPlant(new Vegetable(new GridPosition(1, 1), "Tomato"));
        garden.addPlant(new Flower(new GridPosition(4, 4), "Daisy"));
    }
    ...
    void testPestElimination() {
        HarmfulPest pest = new HarmfulPest(new GridPosition(0, 0));
        assertTrue(pest.isAlive());
    ...
    void testPestPositionChange() {
        GridPosition originalPos = new GridPosition(0, 0);
        GridPosition newPos = new GridPosition(1, 1);
    ...
    void testPestCausesDamage() {
        Plant plant = garden.getPlant(new GridPosition(0, 0));
    ...
    void testManualTreatmentDecreasesStock() {
        ...
        GardenZone zone1 = garden.getZone(1);
    ...
    void testActivePestCountAtPosition() {
        // Initially no pests
        assertEquals(0, pestControl.getActivePestCountAtPosition(new GridPosition(0, 0)));
        assertEquals(0, pestControl.getActivePestCountAtPosition(new GridPosition(1, 1)));
    }
    ...
    void testZoneInfestationLevelUpdate() {
        GardenZone zone = garden.getZone(1);
    ...
    void testUpdateWithEmptyGarden() {
        GardenPlot emptyGarden = new GardenPlot(3, 3);
    ...
    void testMultipleZones() {
        ...
        for (int i = 1; i <= 9; i++) {
            GardenZone zone = garden.getZone(i);
