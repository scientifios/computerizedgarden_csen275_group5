...
import CSEN275Garden.ui.*;
import CSEN275Garden.util.Logger;
import CSEN275Garden.api.GardenSimulationAPI;
...
public class SmartGardenApplication extends Application {
    ...
    private InfoPanel leftSidebarPanel;
    private InfoPanel rightSidebarPanel;
    ...
    private java.util.Map<GridPosition, Integer> lastPestCounts;
    ...
    private Scene createScene() {
        ...
        leftSidebarPanel = new InfoPanel(InfoPanel.Mode.LEFT_SYSTEMS);
        rightSidebarPanel = new InfoPanel(InfoPanel.Mode.RIGHT_GARDEN);
        ...
        gardenPanel.setPlantSelectionHandler(position -> rightSidebarPanel.setSelectedPlant(position));
        rightSidebarPanel.setPlantSelectionHandler(position -> {
            gardenPanel.setSelectedPlant(position);
            rightSidebarPanel.setSelectedPlant(position);
        });
        ...
        root.setLeft(leftSidebarPanel);
        root.setRight(rightSidebarPanel);
        BorderPane.setMargin(centerPanel, new Insets(10));
        BorderPane.setMargin(leftSidebarPanel, new Insets(10));
        BorderPane.setMargin(rightSidebarPanel, new Insets(10));
    ...
    private void setupToolbarActions() {
        ...
        toolbar.getSpeedComboBox().setOnAction(e -> {
            String speed = toolbar.getSpeedComboBox().getValue().replace("x", "");
    ...
    private void setupPestEventHandlers() {
        pestEventBridge.setHandler(new PestEventBridge.PestAnimationHandler() {
            ...
            @Override
            public void onPestRemoved(GridPosition position, String pestType) {
    ...
    private void updateUI() {
        ...
        try {
            ...
            GardenPlot garden = controller.getGarden();
            if (garden == null) {
            ...
            leftSidebarPanel.getTimeLabel().setText("Day " + engine.getDayCounter() + " | " + engine.getFormattedTime());
            leftSidebarPanel.getStatsLabel().setText(
            ...
            leftSidebarPanel.getWeatherDisplay().updateWeather(weather);
            
            // Update heating status
            CSEN275Garden.system.HeatingSystem.HeatingMode heatingMode = 
                engine.getHeatingSystem().getHeatingMode();
            String heatingText;
            if (heatingMode == CSEN275Garden.system.HeatingSystem.HeatingMode.OFF) {
                heatingText = "Heating: Off";
            } else {
                heatingText = "Heating: " + heatingMode.name() + " (" + 
                              engine.getHeatingSystem().getCurrentTemperature() + "°C)";
            }
            leftSidebarPanel.getHeatingStatusLabel().setText(heatingText);
            
            // Update temperature label
            int currentTemp = engine.getHeatingSystem().getCurrentTemperature();
            leftSidebarPanel.getTemperatureLabel().setText("Current: " + currentTemp + "C");
            ...
            leftSidebarPanel.updateProgressBars(waterProgress, pesticideProgress);

            Map<GridPosition, Integer> activePestsByPosition = buildActivePestMap(engine);
            List<GridPosition> criticalThreatPositions = activePestsByPosition.entrySet().stream()
            ...
            leftSidebarPanel.updateLeftPanel(
                weather,
                currentTemp,
            ...
            rightSidebarPanel.updateGardenInfo(
            ...
            try {
                ...
                CSEN275Garden.util.Logger logger = null;
                ...
                if (logger == null) {
                    logger = CSEN275Garden.util.Logger.getInstance();
                }
                
                if (logger != null) {
                    List<CSEN275Garden.util.Logger.LogEntry> logEntries = 
                    ...
                    for (CSEN275Garden.util.Logger.LogEntry entry : logEntries) {
                        try {
    ...
    private int countPlantsNeedingWater(GardenPlot garden) {
    ...
    private String summarizeSensors(SimulationEngine engine) {
        long activeCount = engine.getGarden().getZones().stream()
            ...
            .filter(sensor -> sensor != null && sensor.getStatus() == CSEN275Garden.system.Sensor.SensorStatus.ACTIVE)
            .count();
    ...
    private String formatHeatingStatus(SimulationEngine engine) {
        ...
        if (mode == CSEN275Garden.system.HeatingSystem.HeatingMode.OFF) {
            return "Off";
        }
    ...
    private String formatCoolingStatus(SimulationEngine engine) {
        ...
        if (mode == CSEN275Garden.system.CoolingSystem.CoolingMode.OFF) {
            return "Off";
        }
    ...
    private String formatCriticalThreats(List<GridPosition> criticalThreatPositions) {
    ...
    private Map<GridPosition, Integer> buildActivePestMap(SimulationEngine engine) {
        Map<GridPosition, Integer> pestsByPosition = new HashMap<>();
    ...
    private void updatePestAnimations(SimulationEngine engine) {
        try {
            ...
            java.util.Map<GridPosition, java.util.List<CSEN275Garden.system.Pest>> currentPests = new java.util.HashMap<>();
            
            // Group pests by position
            for (var pest : allPests) {
                if (pest.isAlive()) {
                    GridPosition pos = pest.getPosition();
